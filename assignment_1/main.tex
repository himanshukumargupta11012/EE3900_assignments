...
So,
\begin{align}
	|H(e^{jw})|=\frac{4\cos(\omega)}{5+4\cos(\omega)}
\end{align}
Yes it is periodic because $\cos$ is periodic function
and period of $\cos$ is $2\pi$.So same for that 
...
\end{align}
So, we can conclude that h(n) is bounded and convergent
\begin{figure}[!ht]
\centering
...
We will find the convolution using topleitz matrix.
For that we have to find topleitz matrix of h(n).
